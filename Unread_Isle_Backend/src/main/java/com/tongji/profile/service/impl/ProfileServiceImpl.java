...
public class ProfileServiceImpl implements ProfileService {
     ...
     *   <li>校验用户存在</li>
     *   <li>校验至少提供一个待更新字段</li>
     *   <li>若提交未读岛号（unreadIsleId），校验唯一性</li>
    ...
    public ProfileResponse updateProfile(long userId, ProfilePatchRequest req) {
        ...
        boolean hasAnyField = req.nickname() != null || req.bio() != null || req.gender() != null
                || req.birthday() != null || req.unreadIsleId() != null || req.school() != null
                || req.tagJson() != null;
        ...
        // 未读岛号唯一性校验：仅在提交且非空时检查（排除自己）
        if (req.unreadIsleId() != null && !req.unreadIsleId().isBlank()) {
            boolean exists = userMapper.existsByUnreadIsleIdExceptId(req.unreadIsleId(), current.getId());

            if (exists) {
                throw new BusinessException(ErrorCode.UNREAD_ISLE_ID_EXISTS);
            }
        }
    ...
    private static User getUser(ProfilePatchRequest req, User current) {
        ...
        if (req.unreadIsleId() != null) {
            patch.setUnreadIsleId(req.unreadIsleId().trim());
        }
        if (req.school() != null) {
