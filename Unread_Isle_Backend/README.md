...
未读岛（Unread_Isle）是一个知识获取与分享社区。本目录包含项目的 Spring Boot 后端服务，完整的项目说明和启动方式请参阅仓库根目录的 `README.md`。
...
- **项目细节与亮点**：
    ...
    - **AI 问答系统**：开发未读岛平台 RAG 知识问答系统，实现用户调用接口→索引检查→向量检索→Prompt 构造→大模型流式生成的全流程，通过合理分块、幂等删除保持单一版本、预索引减少首次提问等待时间等，显著提升用户围绕单篇知文的智能问答效率与准确性。
