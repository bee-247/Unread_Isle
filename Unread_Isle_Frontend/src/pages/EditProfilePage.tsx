...
const EditProfilePage = () => {
  ...
  const displayName = useMemo(
    () => user?.nickname ?? user?.phone ?? user?.email ?? "未读岛用户",
    [user]
  );

  const [nickname, setNickname] = useState<string>(user?.nickname ?? "");
  const [bio, setBio] = useState<string>(user?.bio ?? "");
  const [unreadIsleId, setUnreadIsleId] = useState<string>(user?.unreadIsleId ?? "");
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (unreadIsleId.trim()) payload.unreadIsleId = unreadIsleId.trim();
  ...
  const avatarInitial = (displayName.trim().charAt(0) || "未").toUpperCase();
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        setBio(current.bio ?? "");
        setUnreadIsleId(current.unreadIsleId ?? "");
      ...
      <form className={styles.pageCard} onSubmit={onSubmit}>
        ...
        <div className={styles.grid}>
          ...
          <div className={styles.fieldGroup}>
            ...
            <div className={styles.field}>
              <label className={styles.label} htmlFor="unreadIsleId">未读岛 ID</label>
              <input id="unreadIsleId" className={styles.input} value={unreadIsleId} onChange={e => setUnreadIsleId(e.target.value)} placeholder="用于个性化主页地址" />
            </div>
            <div className={styles.field}>
